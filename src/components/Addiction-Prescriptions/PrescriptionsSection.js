import React from 'react'

import './PrescriptionsSection.css'

const PrescriptionsSection = () => {
  return (
<>

<div className='prescriptions-section container'>
  <div className='prescriptions-section-content'>
    <div className='prescriptions-section-flex'>

      <div className='prescriptions-section-left'>
        <h1>PRESCRIPTIONS ADDICTION</h1>
        <p>Medication prescriptions are a common and effective treatment for many conditions and health complications. We all experience procedures, body aches, and other discomforts that require medication to ease the pain. Many forms of pain relief are available over the counter, but prescription medication refers specifically to those prescribed by your medical health professional.
        <br/>This also means that prescription drugs’ potency and addictive qualities when combined with the availability and prevalence of prescription drugs can create a vicious cycle of addiction and physical dependence, if not taken care of properly. And not all prescriptions are created equal; some are more addictive than others.<br/>Abuse of prescription drugs can range from the use of stimulants (prescribed for ADHD patients) to sedatives/hypnotics (anxiety/sleep treatments), or opioids (for pain relief).<br/>Abuse of prescription medications can be recreational, due to addiction, or both, and the consequences can be severe or fatal.</p>
      </div>

      <div className='prescriptions-section-right'>
        <img src='https://sunset-rehab.s3.us-west-1.amazonaws.com/xanax-addiction.jpg' alt="activities" />
       </div>

    </div>
  </div>
</div>


</>
  )
}

export default PrescriptionsSection
